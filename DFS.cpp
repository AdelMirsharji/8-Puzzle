...
State dfs_search(vector<State> &states,int &num_visit, int &num_exp, int &b_dfs_reached_goal) {
	bool result;

	while(!states.empty()){
		State it_state = states.front();
		
		cout << "State getting expanded in DFS = " << endl;
...
int main() {
	//***********************************README and MENU
	cout << endl << "**************************************************************" << endl;
	cout << "In DFS search, successive moves can take us away from the goal rather than bringing closer.";
	cout <<	" The search of state space tree follows the leftmost path from the root regardless of the initial state.";
	cout <<	" An answer node may never be found in this approach." << endl;
	cout << "The DFS search will starts in 20s:" << endl;
	cout << "**************************************************************" << endl;
	sleep(20);
	...
	cout << "B)DFS search" << endl << endl;
	int b_dfs_number_of_nodes_visited = 0;			//B)DFS visited nodes number
	int b_dfs_number_of_nodes_expanded = 0;			//B)DFS expanded nodes number
	int b_dfs_reached_goal = 0;						//B)checking if it reached goal

	//Initializing A)DFS
	states.push_back(first_state);
	b_dfs_number_of_nodes_visited++;

	State final_b_dfs_state = dfs_search(states, b_dfs_number_of_nodes_visited, b_dfs_number_of_nodes_expanded, b_dfs_reached_goal);
	if (b_dfs_reached_goal == 1)
		cout << "B)DFS search reached the goal!" << endl;
	else
		cout << "B)DFS search didnt reach the goal and terminated!" << endl;

	cout << endl << "Final state in B)DFS search:" << endl;
	final_b_dfs_state.print_board();
	cout << endl << "Number of visited nodes in B)DFS search = " << b_dfs_number_of_nodes_visited << endl << "Number of expanded nodes in B)DFS search = " << b_dfs_number_of_nodes_expanded << endl;
